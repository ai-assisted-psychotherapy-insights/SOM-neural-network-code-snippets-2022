...
def data(names, const=0.9):
    for i, name in enumerate(names):
        ...
        for j, let in enumerate(name):
            ctx = const * ctx + rec  # we add the recent to context (and multiply the previous context with a decay constant)
        ...
        ctx = const*ctx + rec  # for the last letter
        rec = nxt
...
dat = data(names)
# save all the data
np.savetxt("train_data.txt", dat, delimiter=",", fmt="%s")  # the fmt argument is for specifying it is a string
